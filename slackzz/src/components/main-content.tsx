...
const MainContent: FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    return (
        <div className={cn('md:px-2 md:pb-2 md:pt-14 md:h-screen', backgroundColor)}> {/* 这里的backgroundColor是动态的，与第一个值合并，而不是覆盖 */}
            {/* 给一个滚动区域在不同屏幕下设置 margin、全高,并自定义滚动条样式 [&::-webkit-scrollbar-thumb]:rounded-[6px] [&::-webkit-scrollbar-thumb]:bg-foreground/60 [&::-webkit-scrollbar-track:bg-none [&::-webkit-scrollbar]:w-2 */}
            <main className={cn(
